...
file = open(sys.argv[1] + '/X.dat','r')
x_data = (np.loadtxt(file,unpack=True,dtype='float32')).T
file.close()
file = open(sys.argv[1] + '/Y.dat','r')
...
def quadratic_separtor(x0,x1):
    ...
    # return (-1*linear_term - np.sqrt(linear_term * linear_term - 4*square_term*final_const))/(2*square_term)
